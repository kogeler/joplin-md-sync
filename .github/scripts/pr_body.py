...
START_MARKER = "<!-- joplin-md-sync-changelog:start -->"
END_MARKER = "<!-- joplin-md-sync-changelog:end -->"
...
def _run(args: argparse.Namespace) -> None:
    changelog = _read_text(args.changelog, label="CHANGELOG.md", maximum_bytes=MAX_CHANGELOG_BYTES)
    existing = _read_text(
        args.existing_body, label="pull-request body", maximum_bytes=MAX_PR_BODY_BYTES
    )
    args.output.write_text(
        _updated_body(existing, _latest_changelog_section(changelog)), encoding="utf-8"
    )
