...
This is the installation and operations guide. The normative installer and
supervisor behavior is in the [service contract](../contracts/SERVICE.md);
network and credential requirements are in the
[security contract](../contracts/SECURITY.md).
...
loopback. Publish only the Actions namespace unless a remote MCP client is
actually required. A shared public listener is supported when `/mcp` keeps its
separate bearer authentication and TLS; Joplin routes remain private.
