...
class ParserTests(unittest.TestCase):
    ...
    def test_fixed_defaults(self) -> None:
        ...
        self.assertEqual(args.joplin_version, "3.6.2")
        self.assertEqual(args.mcp_version, "1.2.0")
        self.assertEqual(args.sync_interval, 300)
        self.assertFalse(args.allow_remote_mcp)
        self.assertIsNone(args.mcp_auth_token)

    def test_update_without_version_explicitly_means_latest(self) -> None:
        args = installer.build_parser({"HOME": "/tmp/home"}).parse_args(["--update-joplin"])
        self.assertEqual(args.update_joplin, "latest")
    ...
    def test_rejects_port_collision(self) -> None:
        args = installer.build_parser({"HOME": "/tmp/home"}).parse_args(
            ["--update-mcp", "1.2.0", "--api-port", "41185", "--mcp-port", "41185"]
        )
    ...
    def test_update_modes_are_mutually_exclusive(self) -> None:
        args = installer.build_parser({"HOME": "/tmp/home"}).parse_args(
            ["--update-joplin", "3.6.2", "--update-mcp", "1.2.0"]
        )
...
class SecretTests(unittest.TestCase):
    ...
    def test_noninteractive_mcp_token_is_generated_then_preserved(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            env = {"HOME": temporary}
            args = installer.build_parser(env).parse_args(["--non-interactive"])
            paths = installer.build_paths(args, env)
            redactor = SecretRedactor()
            generated = installer.resolve_mcp_auth_token(args, paths, redactor)
            self.assertIsNotNone(generated)
            assert generated is not None
            installer.configure_mcp_auth_token(paths, generated)
            self.assertEqual(
...
class UnitAndPathTests(unittest.TestCase):
    ...
    def test_rendered_unit_has_no_secrets_and_absolute_paths(self) -> None:
        ...
        mcp_content = installer.render_mcp_unit(
            (TOOLS_DIR / "systemd" / "joplin-md-sync-mcp.service").read_text(),
            paths=self.paths,
            api_port=41185,
            mcp_port=8765,
        )
        ...
        unauthenticated = installer.render_mcp_unit(
            (TOOLS_DIR / "systemd" / "joplin-md-sync-mcp.service").read_text(),
            paths=self.paths,
            api_port=41185,
            mcp_port=8765,
        ...
        remote = installer.render_mcp_unit(
            (TOOLS_DIR / "systemd" / "joplin-md-sync-mcp.service").read_text(),
            paths=self.paths,
            api_port=41185,
            mcp_port=8765,
    ...
    def test_rendered_unit_passes_systemd_analyze(self) -> None:
        ...
        mcp_unit = self.root / "joplin-md-sync-mcp.service"
        mcp_unit.write_text(
            installer.render_mcp_unit(
                (TOOLS_DIR / "systemd" / "joplin-md-sync-mcp.service").read_text(),
                paths=self.paths,
        ...
        result = subprocess.run(
            ["systemd-analyze", "verify", mcp_unit, unit],
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    def test_mcp_auth_token_is_stored_protected_and_can_be_disabled(self) -> None:
        ...
        unit = installer.render_mcp_unit(
            (TOOLS_DIR / "systemd" / "joplin-md-sync-mcp.service").read_text(),
            paths=self.paths,
            api_port=41185,
            mcp_port=8765,
...
class DryRunTests(unittest.TestCase):
    ...
    def test_update_only_does_not_resolve_secrets_or_touch_profile(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            env = {
                "HOME": str(root / "home"),
    ...
    def test_mcp_update_only_does_not_resolve_secrets_or_lock_profile(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            env = {
                "HOME": str(root / "home"),
            ...
            with (
                ...
                tool.run()
            update.assert_called_once_with(tool.runner, tool.paths, "1.2.0")
            prompt.assert_not_called()
            self.assertFalse((root / "profile").exists())
...
class ServiceLifecycleTests(unittest.TestCase):
    def test_lingering_and_mcp_token_are_resolved_before_sync_password(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            env = {
                "HOME": str(root / "home"),
            ...
            with (
                ...
                tool.run()
            self.assertEqual(order, ["lingering", "mcp-token", "nextcloud-password"])

    def test_joplin_update_restores_both_previously_active_services(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            env = {
                "HOME": str(root / "home"),
            ...
            self.assertEqual(
                ...
                [
                    ("stop", "joplin-md-sync-mcp.service"),
                    ("stop", "joplin-terminal.service"),
                    ("restart", "joplin-terminal.service"),
                    ("restart", "joplin-md-sync-mcp.service"),
                ],
            )
    ...
    def test_install_uses_isolated_global_prefix(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                actual = installer.install_or_update_joplin(runner, dependencies, paths, "3.6.2")
...
class SingleFileDownloadTests(unittest.TestCase):
    def test_downloaded_installer_loads_companion_assets(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("--update-joplin", help_result.stdout)
            ...
            mcp_asset_result = subprocess.run(
                [
                    ...
                    (
                        ...
                        "'systemd/joplin-md-sync-mcp.service')))"
                    ),
                ],
                cwd=root,
                env=env,
