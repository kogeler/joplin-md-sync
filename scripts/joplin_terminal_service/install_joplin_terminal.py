...
def build_parser(env: Mapping[str, str] | None = None) -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "install the released MCP standalone, and create systemd user services."
        )
    )
    ...
    parser.add_argument(
        ...
        const="latest",
        metavar="VERSION",
        help="update only; VERSION defaults to the explicitly selected npm latest tag",
    )
    parser.add_argument(
        "--update-mcp",
        nargs="?",
        const="latest",
        metavar="VERSION",
        help="update only the MCP standalone; VERSION defaults to GitHub latest",
    )
...
def validate_args(args: argparse.Namespace) -> None:
    target_version = args.update_joplin or args.joplin_version
    validate_version(target_version, "Joplin version")
    validate_version(args.update_mcp or args.mcp_version, "MCP version")
    if args.update_joplin is not None and args.update_mcp is not None:
        raise ToolError("--update-joplin and --update-mcp are mutually exclusive")
    ...
    if args.update_joplin is not None or args.update_mcp is not None:
        return
...
def check_dependencies(runner: CommandRunner) -> Dependencies:
    if sys.version_info < (3, 14):
        raise ToolError(f"Python 3.14 or newer is required; found {sys.version.split()[0]}")
    missing = [
        name for name in ("node", "npm", "systemctl", "loginctl") if shutil.which(name) is None
    ]
    if missing:
        raise ToolError(f"missing required program: {', '.join(missing)}")
    node = resolved_path(shutil.which("node") or "node")
    npm = resolved_path(shutil.which("npm") or "npm")
    systemctl = resolved_path(shutil.which("systemctl") or "systemctl")
    loginctl = resolved_path(shutil.which("loginctl") or "loginctl")
    ...
    try:
        ...
        raise ToolError(f"could not parse Node.js version: {node_version!r}") from None
...
def mcp_binary_version(runner: CommandRunner, binary: Path) -> str:
    ...
    if result.returncode != 0:
        raise ToolError(f"refusing non-working joplin-md-sync executable: {binary}")
    try:
...
def run_sync(runner: CommandRunner, paths: InstallPaths, label: str) -> None:
    LOG.info("%s", label)
    runner.run(joplin_command(paths, "sync"), timeout=SYNC_TIMEOUT)
...
def dry_run_plan(
    ...
    if args.update_mcp is not None:
        LOG.info("dry-run: would update MCP standalone to %s", args.update_mcp)
        LOG.info("dry-run: would preserve Joplin profile %s", paths.profile_dir)
        return
    if args.update_joplin is not None:
        LOG.info("dry-run: would update isolated Joplin to %s", args.update_joplin)
    ...
    if not args.no_start_service:
        LOG.info("dry-run: would restart both services and verify /ping and /mcp")
...
class Installer:
    ...
    def run(self) -> None:
        ...
        joplin_version = self.args.update_joplin or self.args.joplin_version
        mcp_version = self.args.update_mcp or self.args.mcp_version
        ...
        if self.args.update_mcp is not None:
            ...
            version = install_or_update_mcp(self.runner, self.paths, mcp_version)
            LOG.info("verified joplin-md-sync MCP standalone %s", version)
            if mcp_was_active and not self.args.no_start_service:
                ...
                LOG.info("%s remains stopped due to --no-start-service", MCP_SERVICE_NAME)
            return

        requested_mcp_auth_token: str | None = None
        if self.args.update_joplin is None:
            configure_lingering(
                self.runner,
        ...
        if self.args.update_joplin is not None:
            if terminal_was_active and not self.args.no_start_service:
                systemctl_command(self.runner, dependencies.systemctl, "restart", SERVICE_NAME)
                wait_for_api(self.args.api_port)
            elif terminal_was_active:
                LOG.info("%s remains stopped due to --no-start-service", SERVICE_NAME)
            if mcp_was_active and not self.args.no_start_service:
                token = load_mcp_auth_token(self.paths, self.redactor)
                systemctl_command(
                    self.runner,
