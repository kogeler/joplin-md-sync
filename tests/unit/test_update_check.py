...
from joplin_md_sync import __version__, update_check
...
class ParseVersionTest(unittest.TestCase):
    def test_valid(self):
        parts = (1, 2, 3)
        plain = ".".join(str(part) for part in parts)
        self.assertEqual(update_check.parse_version(f"v{plain}"), parts)
        self.assertEqual(update_check.parse_version(plain), parts)
...
class CheckForUpdateTest(unittest.TestCase):
    def test_outdated_detected(self):
        current = update_check.parse_version(__version__)
        assert current is not None
        newer = f"v{current[0] + 1}.0.0"
        with mock.patch.object(update_check, "_fetch_json", return_value={"tag_name": newer}):
            result = update_check.check_for_update()
        self.assertTrue(result["outdated"])
        self.assertIn(newer, result["update_command"])
